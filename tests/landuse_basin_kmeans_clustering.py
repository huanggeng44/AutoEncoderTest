...
def cluster_basin_vectors(csv_path, n_clusters=6, output_dir=None):
    ...
    features = np.array([[parse_tensor_value(row[col]) for col in feature_cols] 
    ...
    # 保存结果
    output_path = output_dir / 'landuse_cluster_results.csv'
...
if __name__ == "__main__":
    # 输入文件路径
    input_csv = Path(__file__).parent.parent / "outputs" / "Encoder_output" / "landuse_encoder_output.csv"
