...
# def train_test_split(X_df, Y_df, test_size=0.8):
#     X_n_bands = len(X_df.columns) - 1
#
#     # 计算分割索引（前80%的位置）
#     split_idx = int(test_size * X_n_bands)
#
#     # 构建训练集和测试集的列名列表
#     X_train_cols = [X_df.columns[0]] + X_df.columns[1:1 + split_idx].tolist()
#     X_test_cols = [X_df.columns[0]] + X_df.columns[1 + split_idx:].tolist()
#     Y_train_cols = [Y_df.columns[0]] + Y_df.columns[1:1 + split_idx].tolist()
#     Y_test_cols = [Y_df.columns[0]] + Y_df.columns[1 + split_idx:].tolist()
#
#     # 提取训练集和测试集
#     X_train_df = X_df[X_train_cols]
#     X_test_df = X_df[X_test_cols]
#     Y_train_df = Y_df[Y_train_cols]
#     Y_test_df = Y_df[Y_test_cols]
#
#     # 验证形状
#     print("训练集形状:", X_train_df.shape)  # 应为 (31, 1 + split_idx)
#     print("测试集形状:", X_test_df.shape)  # 应为 (31, 1 + (n_bands - split_idx))
...
class Encoder(nn.Module):
    def __init__(self, device, encoder_input_size=300, hidden_size=600, num_layers=4, encoder_output_size=150):
        super().__init__()
        self.device = device
        self.hidden_layer_size = hidden_size
        self.n_layers = num_layers
        self.embedding_size = encoder_input_size
        self.output_size = encoder_output_size
        self.lstm = nn.LSTM(encoder_input_size, hidden_size, num_layers, batch_first=True)
        # 修改线性层的输出维度
        self.linear = nn.Linear(hidden_size, encoder_output_size)
        self.tanh = nn.Tanh()
...
class Decoder(nn.Module):
    def __init__(self, device, encoder_output_size, hidden_size, num_layers, encoder_input_size):
        super().__init__()
        self.device = device
        self.hidden_layer_size = hidden_size
        self.n_layers = num_layers
        ...
        self.output_size = encoder_input_size
        self.lstm = nn.LSTM(encoder_output_size, hidden_size, num_layers, batch_first=True)
        # 输出维度为 Encoder 的输入维度
        self.linear = nn.Linear(hidden_size, encoder_input_size)
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
    ...
    def forward(self, x, hidden_in, cell_in):
        ...
        # prediction: [seq length, batch size, input_size]
        prediction = self.relu(self.linear(lstm_out))
        return prediction, hidden, cell

class Seq2Seq(nn.Module):
    def __init__(self, device, encoder_input_size, hidden_size, n_layers, encoder_output_size=150):
        super(Seq2Seq, self).__init__()
        self.device = device
        # 使用现有的 Encoder 和 Decoder 类
        self.encoder = Encoder(device, encoder_input_size, hidden_size, n_layers, encoder_output_size)
        # 传递 encoder_output_size 和 input_size 给 Decoder
        self.decoder = Decoder(device, encoder_output_size, hidden_size, n_layers, encoder_input_size)
...
# 3.模型训练
def train(model, train_dataloader, test_dataloader, optimizer, criterion_train, criterion_test, device, N_EPOCHS, training_log_file_path, testing_log_file_path, train_batch_loss_file_path, test_batch_loss_file_path):
    model.train()
    train_epoch_list = []
    train_loss_list = []
    test_epoch_list = []
    ...
    for epoch in range(N_EPOCHS):
        ...
        for batch in train_dataloader:
            ...
            loss = criterion_train(output, trg)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for batch in test_dataloader:
                ...
                test_batch_losses.append(test_batch_loss)  # 保存当前批次的损失
                total_loss += test_batch_loss

        test_loss = total_loss / len(test_dataloader)
        test_epoch_list.append(epoch)
        test_loss_list.append(test_loss)
        test_all_batch_losses.extend(test_batch_losses)  # 追加当前 epoch 的批次损失
        print(f'Epoch {epoch + 1}, Test Loss: {test_loss:.4f}')

        # 每个 epoch 后实时保存训练损失到 CSV
        ...
        # 每个 epoch 后实时保存测试损失到 CSV
        with open(testing_log_file_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['epoch', 'test_loss'])  # 写入表头
    ...
    return train_epoch_list, train_loss_list,test_epoch_list,  test_loss_list
...
def main():
    ...
    basin_timeserious_type = "ROF"
    basin_timeserious_type_name = "径流量"
    basin_timeserious_unit = "mm"
    basin_file = rf'F:\AutoEncoderDatasets\TimeseriousData\{basin_timeserious_type}_{basin_timeserious_unit}_sub_ave.shp'
    ...
    # X_train, X_test, y_train, y_test = train_test_split(runoff_normalized, runoff_normalized, test_size=0.5)
    ...
    train_encoder_input_size = len(runoff_timeserious_list_train[0])    # 表示输入序列的特征维度，即每个时间步的特征数量，这里等于T 。
    hidden_size = train_encoder_input_size    # 隐藏层的维度，通常用于表示编码器对输入序列的编码表示。
    n_layers = 1    # 循环神经网络的层数。
    encoder_output_size = 150  # 降维后的输出维度
    ...
    if not model_fname is None:
        ...
        model = Seq2Seq(device, train_encoder_input_size, hidden_size, n_layers, encoder_output_size).to(device)


    # 训练循环
    criterion_train = nn.MSELoss()
    criterion_test = nn.MSELoss()  # 测试集使用相同的损失函数
    ...
    # 开始训练并获取损失值
    train_epoch_list, train_loss_list, test_epoch_list, test_loss_list = train(model, train_loader, test_loader, optimizer, criterion_train, criterion_test, device, N_EPOCHS, training_log_file_path, testing_log_file_path, train_batch_loss_file_path, test_batch_loss_file_path)

    # 保存训练好的模型
    model_save_name = f"{basin_timeserious_type}_model_step_{N_EPOCHS}_ppl_{train_loss_list[-1]}.pth"
    ...
    # 加载完成训练后的model
    allbasin_encoder_input_size = len(runoff_timeserious_list[0])
    model_trained = Seq2Seq(device, allbasin_encoder_input_size, hidden_size, n_layers, encoder_output_size).to(device)
    ...
    print(len(encoder_output.keys()))
    # 遍历每个键并检查维度
    for key in encoder_output.keys():
        value = encoder_output[key]
        if isinstance(value, torch.Tensor):
            print(f"键 '{key}' 的维度: {value.shape}")
        else:
            print(f"键 '{key}' 对应的值不是张量（类型为 {type(value)}）")
